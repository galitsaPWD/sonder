...
// Normalize billing_period to consistent "Month Year" display
...
async function loadReadingList(options = {}) {
    ...
    try {
        ...
        let listData = customers.map(c => ({
            ...c,
            reading: billMap[c.id] || { previous_reading: '--', current_reading: '--', consumption: '--', updated_at: null }
        }));
        ...
        listData.sort((a, b) => {
            ...
            if (sortBy === 'updated_at') {
                valA = a.reading.updated_at;
                valB = b.reading.updated_at;
                
                // Handle nulls: Always push to bottom regardless of order
        ...
        tbody.innerHTML = listData.map(item => {
            ...
            const dateRead = item.reading.updated_at ? formatLocalDateTime(item.reading.updated_at) : '--';
...
async function loadRecentReadingsWidget() {
    ...
    try {
        ...
        tbody.innerHTML = readings.map(r => {
            ...
            const date = formatLocalDateTime(r.updated_at);
